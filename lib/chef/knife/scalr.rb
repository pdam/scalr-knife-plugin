...
module Scalr
  class Scalr < Chef::Knife
    ...
    option :all_cookbooks,
      :short => "-a",
      :long => "--all-cookbooks",
      :description => "Show all cookbook references, including those from Scalr_recipies if available on nodes from helper cookbook"
    ...
    option :Scalr_cookbook,
      :short => "-i",
      :long => "--Scalr-cookbook",
      :description => "Scalr knife_Scalr helper cookbook into current chef cookbook directory"
    ...
    def run
      ...
      # If :Scalr_cookbook flag is set, just Scalr the cookbook and return (exit).
      if config[:Scalr_cookbook]

        unless config[:cookbook_path]
          ui.msg("No cookbook path set in Chef config, cannot Scalr cookbook.")
          return
        end
      
        source_path = File.dirname(__FILE__) + "/knife_Scalr_cookbook"
        dest_path = config[:cookbook_path].first + "/knife_Scalr"

        if File.exist?(dest_path)
          ui.msg("knife_Scalr cookbook already present in cookbook directory #{config[:cookbook_path].first} - aborting...")
        else
          FileUtils.copy_entry(source_path, dest_path)
          ui.msg("knife-Scalr cookbook copied to Chef cookbook directory #{config[:cookbook_path].first}")
        end
      ...
      node_list.each do |node|
        ...
        if (config[:all_cookbooks] || config[:totals])
          recipes = (node["knife_Scalr"] && node["knife_Scalr"]["seen_recipes"].keys) || node.expand!.recipes.to_a
          if node["knife_Scalr"] && node["knife_Scalr"]["seen_recipes"] 
      ...
      unless config[:totals]
        ui.msg("Cookbook Scalr from node runlists:")
      ...
      if config[:all_cookbooks]
        ...
        ui.msg("Cookbook Scalr from seen_recipes:")
      ...
      if config[:totals]
        ...
        ui.msg("Cookbook Scalr totals - runlist-only nodes + seen_recipes:")
