...
class Chef
  class Knife
    class scalrServerManage < Knife
      ...
      banner "knife scalr server create (options)"
      ...
      option :scalr_metadata,
        :short => "-M JSON",
        :long => "--scalr-metadata JSON",
        :description => "JSON string version of metadata hash to be supplied with the server create call",
        :proc => Proc.new { |m| Chef::Config[:knife][:scalr_metadata] = JSON.parse(m) },
        :default => ""
      ...
      def run
        ...
        server = connection.servers.create(
          ...
          :metadata => Chef::Config[:knife][:scalr_metadata]
          )
